import streamlit as st #untuk front and sistem
from streamlit_option_menu import option_menu # navbar
import pandas as pd # library dataframe
import numpy as np # untuk komputasi matematika (masukkan library untuk memakukkan komputasi matematika komputer (perhitungan menjumlahkan data di list)
import sqlite3 # library untuk database
import random # untuk generate angka random (library GA)
import time # untuk mengambil current time (mengetahhui waktu)
...
def make_kromosom():
    ...
    for i in range(n_kelas):
        ...
        else: # disini untuk kelas ips yg jumlah nya 10 kelas 
        ...
        kromosom.append(gen)
    return kromosom # kalo sudah, di return
...
def mutasi(i, mapel_ipa, mapel_ips, waktu_sesi_ipa, waktu_sesi_ips, data_mapel, guru_pengampu):
    ...
    return gen # kalo sudah, di return 
    ...
    for i in range(len(temp)): # cek panjang list temp, harusnya bakal ada 24 (mewakili total kelas kita)
        ...
        if i != 0:
            ...
            for j in id_guru: # disini perulangan untuk ngecek id guru yaa
                ...
                # kalo id guru itu tadi muncul lebih dari 1, berarti kan lebih jam gurunya
                if temp[i].count(j) > 1:
                    hasil += hard_bc # nah kalo udah lebih, kasih SP, berupa penambahan bobot constrainnya 0.035
...
def func_fitness(populasi):
    ...
    return fitness # terus di return
...
def penjadwalan(cr, mr, popsize, max_iter):
    ...
    while best != 1 and generasi <= max_iter: # oke skrang jalankan GA sebanyak max_iter yang diinginkan
        ...
        # di sini kita hitung, mau buat offspring/anak berapa
        # maksutnya kita hitung, berapa banyak ortu yg bisa kawin silang
        offspringcross = int(popsize * cr) # 4
        cross = list() # buat list kosong dulu 
        ...
        for i in sorting[offspringcross:]: # terus ambil ortu ortu terpilih tadi
            ...
            # jadi disini kita isolasi mereka biar kawin silang
        ...
        anak = list() # sekarng buat list untuk nampung hasil crossover
        for i in range(len(cross)): # perulangan sebanyak ortu terpilih
            ...
            born[ortu[i]] = born_ # hasil dari proses crossover ini akan menghasilkan anak yg baru dari proses crossover
            # kita kasih nama born 
        ...
        for i in range(len(born)):
            ...
            # kalo lebih besar mr maka mutasi boleh dilakukan, kalo sebliknya yaa jangan harap mutasi
        ...
        # karna udah lebih baik dan yaa berhak untuk dianggap sbg suatu individu dalam populasi

        for i in range(len(anak)): # untuk setiap anak
            # cek nilai fitnessnya, apakah lebih besar dari populasi ortunya
            if func_fitness([populasi[sorting.index(sorting[i])]])[0] <= func_fitness([anak[i]])[0]:
                # kalau ternyata lebih bagus si anak
                # berarti ortu tidak bagus dan kita gantikan dengan si anak
        ...
        if best_fitness < best: # disini branching buat kita simpen individu terbaik
            ...
            populasi_final = populasi # trus ambil juga individu tadi
...
with st.container():
 ...
 # tampilkan judul sistem
        ...
        st.subheader("Tampilan Hasil Skenario Uji Percobaan")
       ...
       # percobaan_sistem = pd.DataFrame(percobaan, columns=['Cr','Mr','Popsize','Generasi','Best Fitness','Average Fitness','msg'])
        ...
        if button_apply:
            ...
            # df_jadwal.to_csv(r'C:\main\Penjadwalan GA1.csv', index=False)
        ...
        if not button_apply:
            try:
                read = pd.read_csv(r'C:\main\Penjadwalan GA.csv')
